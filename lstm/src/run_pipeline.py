"""
CLI 실행 스크립트: feature_pipeline_v2 피처 빌드를 cmd에서 실행

라벨링은 별도 단계(scripts/label_turtle.py + src/attach_labels.py)에서 수행되며,
본 스크립트는 X / meta / df_full(피처만)을 생성한다.
...
사용법:
    ...
    python run_pipeline.py --data_dir ./data --start_date 2010-01-01 ^
        --min_turnover_mn 1000 --min_mktcap 5e10 --output_dir ./output
...
출력 파일 (output_dir 폴더에):
    ...
    df_full.parquet    - 피처 통합 (라벨은 attach_labels.py가 부착)
    summary.txt        - 실행 요약
"""
...
from feature_pipeline_v2 import (
    ...
    build_cross_sectional_features, prepare_train_data,
)
...
def main():
    ...
    print("라벨링    : 본 단계에서 미실행 (attach_labels.py에서 부착)")
    print("=" * 60)
    ...
    # --- 5. X / meta 분리 (라벨은 attach_labels.py에서 부착) ---
    t0 = time.time()
    X, _, meta = prepare_train_data(df, label_col=None)
    print(f"\n  ⏱ X/meta 분리: {time.time()-t0:.1f}s")
    ...
    if not args.no_save:
        ...
        # 통합 파일 (분석용). 라벨 컬럼 부착은 attach_labels.py가 처리.
        ...
        with open(output_dir / 'summary.txt', 'w', encoding='utf-8') as f:
            ...
            f.write(f"\n라벨링: 본 단계 미실행 (attach_labels.py가 부착)\n")
            f.write(f"\nX shape: {X.shape}\n")
            f.write(f"\n피처 목록:\n")
            for c in X.columns:
                f.write(f"  - {c}\n")
        ...
        print(f"\n[저장 완료] {output_dir}")
        for fname in ['X.parquet', 'meta.parquet', 'df_full.parquet', 'summary.txt']:
            fp = output_dir / fname
