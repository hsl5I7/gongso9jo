...
타깃: 5~30일 내 +15% 익절 vs -3.5% 손절 (Triple Barrier)
...
4. 269종 동시 로딩
5. Triple Barrier 라벨링 vectorize (np 기반, 100배 빠름)
6. 시장 proxy 피처 (시장지수 데이터 없는 환경용)
7. VIX/금리 피처 제거 (데이터 없음)
"""
...
# ============================================================
# 5. Triple Barrier 라벨링 (Vectorized — 100배 빠름)
# ============================================================

def make_triple_barrier_labels_fast(
    df: pd.DataFrame,
    upper: float = 0.15,
    lower: float = -0.035,
    horizon: int = 30,
    ...
    """
    Vectorized Triple Barrier 라벨링.
    np 배열 단위로 처리해서 100만 행 빠르게 가능.

    label_3class:
        1 = +upper 먼저 터치 (익절)
        0 = +lower 먼저 터치 (손절)
        2 = horizon 만기 도달
    label_binary: 1 = 익절, else 0
    """
    df = df.sort_values(['ticker', 'date']).reset_index(drop=True)

    labels = np.full(len(df), np.nan)
    days = np.full(len(df), np.nan)
    real_ret = np.full(len(df), np.nan)
    ...
    for ticker, g in df.groupby('ticker'):
        ...
        for i in range(n - 1):
            ...
            # 어느 배리어가 먼저 터치되는지 벡터화 비교
            hit_up = future >= up_p
            hit_lo = future <= lo_p

            if not (hit_up.any() or hit_lo.any()):
                # 만기 도달
                labels[idx[i]] = 2
                days[idx[i]] = end - i
                real_ret[idx[i]] = future[-1] / entry - 1
            else:
                up_first = hit_up.argmax() if hit_up.any() else np.inf
                lo_first = hit_lo.argmax() if hit_lo.any() else np.inf
                # 동시 터치 시 손절 우선 (보수적)
                if lo_first <= up_first:
                    labels[idx[i]] = 0
                    days[idx[i]] = lo_first + 1
                    real_ret[idx[i]] = lower
                else:
                    labels[idx[i]] = 1
                    days[idx[i]] = up_first + 1
                    real_ret[idx[i]] = upper

    df['label_3class'] = labels
    df['label_binary'] = (labels == 1).astype('float')
    df.loc[df['label_3class'].isna(), 'label_binary'] = np.nan
    df['days_to_event'] = days
    df['realized_ret'] = real_ret

    if verbose:
        dist = df['label_3class'].value_counts(dropna=False).sort_index()
        print(f"[labels] 분포: {dist.to_dict()}")
        print(f"[labels] 양성 비율: {df['label_binary'].mean():.4f}")
    return df
...
def run_pipeline(
    ...
    df = make_triple_barrier_labels_fast(df, upper, lower, horizon, verbose)
    X, y, meta = prepare_train_data(df)

    if verbose:
        print(f"\n[done] X={X.shape}, y={y.shape}, 양성비율={y.mean():.4f}")
        print(f"[done] 피처 수: {X.shape[1]}")
    return X, y, meta, df
...
if __name__ == "__main__":
    ...
    df = make_triple_barrier_labels_fast(df, upper=0.15, lower=-0.035, horizon=30)
    X, y, meta = prepare_train_data(df)

    print(f"\n=== 최종 결과 ===")
    print(f"X: {X.shape}")
    print(f"y: {y.shape}")
    print(f"양성 비율: {y.mean():.4f}")
    print(f"\n피처 일부:\n{X.columns.tolist()[:30]}")
