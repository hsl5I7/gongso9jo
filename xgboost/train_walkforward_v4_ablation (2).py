...
def main():
    ...
    p.add_argument("--exclude_yt", action="store_true",
                   help="유튜버 시그널 컬럼(sig_*)을 피처에서 제외하고 학습 "
                        "(베이스라인 Top70만 사용)")
    p.add_argument("--keep_a_only", action="store_true",
                   help="유튜버 시그널 중 A그룹(영매공파) 7개만 남기고 "
                        "B/C/D/E 22개는 제외 (Top70 + A시그널7 = 77개)")
    args = p.parse_args()
    if args.exclude_yt and args.keep_a_only:
        raise SystemExit("[오류] --exclude_yt 와 --keep_a_only 는 동시 사용 불가")
    ...
    if args.exclude_yt:
        ...
        if len(sig_cols) == 0:
            print(f"  [exclude_yt] sig_* 컬럼이 없음. 그대로 진행")
        else:
            X = X.drop(columns=sig_cols)
            print(f"  [exclude_yt] sig_* {len(sig_cols)}개 컬럼 제외")
            print(f"    제외된 컬럼: {sig_cols}")
            print(f"  X (제외 후): {X.shape}")

    # A그룹(영매공파)만 남기는 옵션 — B/C/D/E 제외
    if args.keep_a_only:
        # A그룹 = 영매공파 7개 (개별 5 + AND + strength)
        a_group_cols = {
            'sig_yokbae', 'sig_maejip', 'sig_gonguri', 'sig_paran',
            'sig_ma112', 'sig_yg_all', 'sig_yg_strength',
        }
        sig_cols = [c for c in X.columns if c.startswith('sig_')]
        to_drop = [c for c in sig_cols if c not in a_group_cols]
        a_kept = [c for c in sig_cols if c in a_group_cols]
        if len(sig_cols) == 0:
            print(f"  [keep_a_only] sig_* 컬럼이 없음. 그대로 진행")
        else:
            X = X.drop(columns=to_drop)
            print(f"  [keep_a_only] A그룹(영매공파) {len(a_kept)}개만 유지, "
                  f"B/C/D/E {len(to_drop)}개 제외")
            print(f"    유지된 A컬럼: {a_kept}")
            print(f"    제외된 컬럼: {to_drop}")
            print(f"  X (제외 후): {X.shape}")
    ...
    # 저장 (파일명에 피처 구성 태그 자동 부여)
    #   - 유튜버 제외(70개)        : _70feat_noYT
    #   - 유튜버 포함(99개, 전부)  : _99feat_full
    #   - A그룹만(77개, 영매공파만): _77feat_Aonly
    if args.exclude_yt:
        tag = "70feat_noYT"
    elif args.keep_a_only:
        tag = "77feat_Aonly"
    else:
        tag = "99feat_full"
    print(f"\n[출력 태그] {tag}  (파일명에 자동 부여)")

    summary_df.to_csv(output_dir / f'walk_forward_summary_{tag}.csv', index=False)
    print(f"저장: {output_dir / f'walk_forward_summary_{tag}.csv'}")

    # 마지막 fold 모델 저장
    last = fold_results[-1]
    last['model'].save_model(output_dir / f'last_model_{tag}.json')

    # 거래 로그 (top mode)
    all_trades = pd.concat([r['sim_results']['top']['trades']
                            for r in fold_results], ignore_index=True)
    all_trades.to_csv(output_dir / f'all_trades_top_{tag}.csv', index=False)
    print(f"저장: {output_dir / f'all_trades_top_{tag}.csv'} ({len(all_trades)} trades)")
    ...
    for r in fold_results:
        ...
        # fold별 저장
        pred.to_parquet(
            pred_dir / f"predictions_fold_{r['test_year']}_{tag}.parquet",
            index=False
        )
        all_preds.append(pred)
    # 전체 통합
    all_preds_df = pd.concat(all_preds, ignore_index=True)
    all_preds_df.to_parquet(output_dir / f'predictions_all_{tag}.parquet', index=False)
    print(f"저장: {output_dir / f'predictions_all_{tag}.parquet'} "
